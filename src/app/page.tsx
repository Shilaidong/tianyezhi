...
import EyesStrip from "@/components/eyes-strip";
...
export default async function Home() {
  ...
  const halves = posts.filter((post) => !used.has(post.slug)).slice(0, 2);
  halves.forEach((p) => used.add(p.slug));
  const latest = posts.filter((post) => !used.has(post.slug)).slice(0, 6);
