...
export default function IssueDeck({ issues }: { issues: Issue[] }) {
  ...
  return (
    <section className="issue-deck-section">
      <div className="oa-container">
        <div className="flex items-baseline justify-between border-b border-ink pb-4">
          <div>
            <h2 className="oa-section-title">特刊</h2>
            <span className="oa-native oa-native--en">The Issues</span>
          </div>
          <Link href="/issues" className="oa-label text-ink-soft transition-colors hover:text-seal">
            全部特刊 <IconArrowRight className="oa-arrow" />
          </Link>
        </div>
      </div>
      ...
      >
        {ordered.map((issue, index) => {
          ...
          const style = {
            zIndex: 20 - abs,
            transform: `translate(-50%, -50%) translate3d(${offset * 42}%, ${abs * 12}px, ${-abs * 110}px) rotateY(${offset * -18}deg) scale(${1 - abs * 0.13})`,
            opacity: abs === 2 ? 0.4 : 1,
      ...
      <div className="issue-deck-meta oa-container">
