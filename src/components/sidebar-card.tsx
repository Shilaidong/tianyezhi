...
export default function SidebarCard({ post }: { post: Post }) {
  ...
  return (
    <Link href={`/posts/${post.slug}`} className="group block">
      <p className="oa-label text-black">{section?.name}</p>
      <CoverPhoto
        image={post.image}
        motif={post.motif}
        alt={post.title}
      ...
      <p className="mt-2 line-clamp-2 font-body text-[15px] font-light leading-snug text-black">
        {post.dek}
      </p>
      <p className="oa-label mt-2 text-black">文 / {post.author}</p>
    </Link>
  );
}
